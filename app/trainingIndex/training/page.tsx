...
function SearchParamsComponent({ onParamsChange }: { onParamsChange: (params: { level: string; key: string; mm: string }) => void }) {
  ...
  useEffect(() => {
    ...
    onParamsChange({ level: level ?? "", key: key ?? "", mm: mm ?? "" });
...
const TrainingPage = () => {
  ...
  // 検索パラメータの変更を処理する関数
  const handleParamsChange = ({ level, key, mm }: { level: string; key: string; mm: string }) => {
    setLevel(level);
    setKey(key);
    setMm(mm);
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    setLoading(true);
    if (diatonicChords.length > 0 && level) {
      getUseChords();
      if(!loading){
  ...
  return (
    <div>
      ...
      <Suspense fallback={<div>Loading...</div>}>
        {/* 検索パラメータを使用するコンポーネントをSuspenseでラップ */}
